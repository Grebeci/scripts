#!/bin/bash
###########################################################################
####  fork 自 https://github.com/skiinder/scripts/blob/master/myshell/init.sh
####  快速搭建一个数仓环境
#############################################################################
...
function depend_on() {
  for i in $@; do
    case $i in
    "env")
      su - xin -c "which pv >/dev/null 2>&1" || init_env $CLUSTER
      ;;
    "java"|"mysql")
      su - xin -c "which $i >/dev/null 2>&1" || init_$i $CLUSTER
      ;;
...
sed -i '/<azkaban-users>/a<user password="xin" roles="metrics,admin" username="xin"\/>' $AZHOME/azkaban-web/conf/azkaban-users.xml
prt "生成控制脚本az.sh"
...
add_property /opt/module/yanagishima-18.0/yanagishima.properties "presto.datasources" "xin-presto"
add_property /opt/module/yanagishima-18.0/yanagishima.properties "presto.coordinator.server.xin-presto" "http://hadoop102:8881"
add_property /opt/module/yanagishima-18.0/yanagishima.properties "catalog.xin-presto" "hive"
add_property /opt/module/yanagishima-18.0/yanagishima.properties "schema.xin-presto" "default"
